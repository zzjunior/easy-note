...
class BoardManager {
    ...
    // CRUD de Cards
    openCardModal(listId, cardId = null, cardData = null) {
        ...
        if (cardId && cardData) {
            ...
            listIdInput.value = cardData.list_id;
        } else {
            // Criando novo card
    ...
    async handleCardSubmit(e) {
        ...
        console.log('handleCardSubmit - dados:', { cardId, title, description, listId });
        ...
        try {
            ...
            console.log('Enviando request:', { url, method, body });
    ...
    editCard(cardId) {
        // Carrega o card e abre modal de edição
    ...
    async loadCardForEdit(cardId) {
        try {
            ...
            const card = await response.json();
            this.openCardModal(card.list_id, cardId, card);
            
        } catch (error) {
            console.error('Erro ao carregar card para edição:', error);
            this.showError('Erro ao carregar card para edição');
        }
    }
